...
并提供不携带私有应用路由的独立静态构建。Public Agent 只读取版本化公开 corpus，
返回 citation 与 package receipt；它使用独立凭据、独立容器和同源单路由，服务异常时会透明
回退到本页公开资料。它不是私人 Harness 的匿名入口，稳定 HTTPS 地址将在正式部署后补充到这里。
...
- 公开主页已有独立静态构建、受限 Public Agent、同 SHA 双镜像候选探活与共同回滚；正式
  上线仍要求服务器注入专用公开模型凭据并通过真实 provider smoke，且永不共享主对话权限。
