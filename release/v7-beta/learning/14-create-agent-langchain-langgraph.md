...
> 本章目标：能在面试中讲清 LangChain 和 LangGraph 的区别、`create_agent` 内部做了什么、StateGraph + checkpointer + middleware 的协作机制、以及 Sage 为什么选这套而不是自研 XML 协议。
>
> 这是面试高频考点。LangChain 和 LangGraph 不是同一个东西，`create_agent` 也不是简单的 ReAct 循环。
...
| `middleware` | 10 个 middleware | 横切逻辑（净化/预算/错误处理） |
...
**这是面试必问的点**：LangGraph checkpointer 和 Sage 的 SessionEventJournal 都做持久化，为什么不合并？
...
不直接替换，双轨共存。通过对等矩阵证明 v2 不劣于 legacy 后才切默认。
...
A: 旧 session 的 history/trace/diff 都是 XML 协议格式，直接切会导致历史 replay 失败。双轨让新旧 runtime 共存，通过对等矩阵证明 v2 不劣于 legacy 后才切默认值。这是渐进式迁移的常见做法。
...
4. `packages/sage_harness/sage_harness/middleware/builtin.py` - 10 个 middleware
...
- `tests/harness/test_run_manager.py` - 执行管理
- `tests/core/harness/test_event_adapter.py` - 事件转换

## 当前边界

> [!warning] create_agent 相关的已知边界
> - 10 个 middleware 实现（设计目标 22 个）
