...
| --- | --- | --- | --- |
| 事件持久化 | SQLite SessionEventJournal | JSONL events | transcript |
| 重连重放 | ✅ 无缝（先重放后订阅） | ❌ 无重连 | ❌ |
| run 持有方 | 服务端 RunCoordinator | 进程内 | 进程内 |
| WebSocket 断开 | run 继续 | run 丢失 | run 丢失 |
| 进程重启 | interrupted + retryable | run 丢失 | run 丢失 |
| 幂等 | event_id UNIQUE | 无 | 无 |
| fencing token | ✅ 防幽灵写入 | 无 | 无 |

Sage 的持久 timeline + 无缝重放 + fencing token 是 Pico/Claude Code 都没有的设计。这是 Web 形态的必然要求--浏览器会断线、会刷新、会切换标签页，run 必须能在这些情况下存活。
...
7. `frontend/src/composables/useCodingSessionRoute.ts` - URL 深链接
