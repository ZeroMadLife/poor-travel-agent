...
`_build_ainvoke_messages()` 按 `SYSTEM_PROMPT_DYNAMIC_BOUNDARY` 把 prompt 拆成 system + user 两条消息（详见 [[04-context-prompt-caching]]）。
...
| --- | --- | --- | --- |
| Runtime/Engine 拆分 | ✅ 两层 | ✅ 两层 | ✅ QueryEngine/query |
| 模型协议 | legacy XML + v2 原生 tool calling | XML `<tool>/<final>` | SDK message + tool block |
| 横切逻辑 | v2 middleware 链（10 个） | ToolExecutor + Policy | permission + hooks |
| 终态归一 | succeeded/failed/cancelled/interrupted/budget_exhausted | final/step_limit/error | done/error/aborted |
| 流式过滤 | `_visible_final_delta()` 只转发 final | 无（不流式） | SDK block 分离 |

Sage 的 v2 middleware 链比 Pico 的 ToolExecutor 更可组合，但实现度不如 Pico 完整（10/22）。
...
6. `packages/sage_harness/sage_harness/middleware/builtin.py` - 10 个 middleware
...
> [!warning] v2 runtime 的 middleware 链还在补齐
> - 实际 10 个 middleware（设计目标 22 个）
> - 未实现：SagePolicy / ApprovalInterrupt / ReadBeforeWrite / Summarization / Todo / Memory / DeferredTool / SubagentLimit / LoopDetection / Clarification
> - 这些功能在 legacy runtime 里由 ToolExecutor + ToolPolicyChecker + ApprovalManager 承担
> - `deerflow_v2` 默认 gate 关闭，群友试用阶段走 legacy
