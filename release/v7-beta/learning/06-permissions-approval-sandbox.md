...
V7-beta 实现了 ContainerSandbox 代码，但**未在目标服务器真实验证**（CPU/内存/PID/网络/workspace mount/终止回收/并发隔离）。这是群友试用前的硬阻塞项之一。
...
| --- | --- | --- | --- | --- |
| 权限层 | PermissionChecker（4 mode） | tool profile + approval | permission context + mode | permission context |
| 策略层 | ToolPolicyChecker | tool policy | hooks | - |
| 审批 | ApprovalManager（11 模式 + 4 级） | approval policy | interactive handler | SmartApprovalManager |
| 危险命令 | 11 个正则模式 | shell 分类 | permission context | CANNOT_AUTO_APPROVE |
| 路径安全 | 5 层（resolve/NOFOLLOW/hardlink/inode/fingerprint） | workspace path | permission context | path_security.py |
| 沙盒 | Local + Container | local sandbox | 无显式沙盒 | sandbox |
| 跨 agent 文件状态 | 设计（FileStateRegistry，未实现） | 无 | FileStateRegistry | FileStateRegistry |

Sage 的五道门设计比 Pico 更细粒度，但跨 agent 文件状态协调（Hermes 的 FileStateRegistry）还没实现。
