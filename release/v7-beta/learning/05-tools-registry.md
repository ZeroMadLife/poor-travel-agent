...
详见 [[06-permissions-approval-sandbox]]。
...
| --- | --- | --- | --- |
| 注册方式 | `@register_tool` 装饰器 | Python registry | 独立工具模块 |
| Schema 验证 | Pydantic + workspace 条件 + Permission/Policy | Pydantic + policy | permission context + hooks |
| 延迟加载 | `deferred=True` + `tool_search` | 无 | 无（全部暴露） |
| 执行隔离 | ThreadPoolExecutor + 超时 | 同进程 + 超时 | 独立进程 |
| 失败归一 | `ToolResult(is_error=True)` | ToolResult | SDK error block |
| 工具数 | 25 | ~10 | 50+ |

Sage 的 deferred loading 是 Pico/Claude Code 没有的设计，解决了"工具多但不想全部暴露"的矛盾。
...
> - `allowed_tools` 在 SKILL.md frontmatter 里解析了但没强制执行（详见 [[07-skills-commands]]）
> - v2 的 tool metadata（risk_level/permission_scope/...）设计完成，部分实现
> - AST 自动工具发现（Hermes 风格）未实现，还是硬编码 TOOL_MODULES
...
下一章：[[06-permissions-approval-sandbox]]
