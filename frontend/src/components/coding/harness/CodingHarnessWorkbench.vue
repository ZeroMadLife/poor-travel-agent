...
const journey = computed<LearningJourney>(() => {
  ...
  if (!props.threadGoal && !['running', 'blocked'].includes(props.projection.status)) return 'contract'
  return 'active'
})
  ...
  >
    ...
    <div class="journey-canvas">
      ...
      <section v-if="journey === 'contract'" class="journey-surface contract-surface">
        ...
        <form class="goal-contract-form" @submit.prevent="submitGoal">
        ...
        <p class="surface-guidance"><ShieldCheck :size="15" />确认后目标会持久化到 Thread；运行时冻结对应 revision，掌握度仍只接受可验证证据。</p>
      </section>
      ...
      <section v-else class="journey-surface active-surface">
        <header class="goal-heading">
          <div><p><Target :size="15" />Current Goal <code v-if="threadGoal">rev {{ threadGoal.revision }}</code></p><h1>{{ goalTitle }}</h1><span>当前 Thread 的学习路径；只有 timeline 与已验证证据会改变状态。</span></div>
...
.goal-heading p code { color:var(--sage-text-muted); font-family:var(--sage-font-mono); font-size:9px; font-weight:500; text-transform:none; }
...
.goal-actions { display:flex; align-items:center; gap:8px; min-height:46px; border-bottom:1px solid var(--sage-border); }.goal-actions button,.goal-contract-form button { min-height:30px; padding:0 11px; border:1px solid var(--sage-border); border-radius:var(--sage-radius); color:var(--sage-text-secondary); background:var(--sage-surface); font-size:10px; cursor:pointer; }.goal-actions button.primary,.goal-contract-form button { border-color:var(--sage-brand-strong); color:var(--sage-surface); background:var(--sage-brand-strong); }.goal-actions button:disabled,.goal-contract-form button:disabled { cursor:not-allowed; opacity:.48; }.goal-actions>span { margin-left:auto; color:var(--sage-text-muted); font-size:9px; }.goal-error { margin:9px 0 0; color:var(--sage-danger); font-size:10px; }
...
.goal-contract-form { display:grid; grid-template-columns:minmax(0,1fr) minmax(0,1fr) auto; align-items:end; gap:12px; padding:18px 0; border-bottom:1px solid var(--sage-border); }.goal-contract-form label { display:grid; gap:6px; }.goal-contract-form small { color:var(--sage-text-muted); font-size:9px; }.goal-contract-form textarea { width:100%; min-height:64px; resize:vertical; padding:9px 10px; border:1px solid var(--sage-border); border-radius:var(--sage-radius); color:var(--sage-text); background:var(--sage-surface); font:inherit; font-size:11px; line-height:1.5; box-sizing:border-box; }.goal-contract-form textarea:focus { border-color:var(--sage-brand-strong); outline:none; }.goal-contract-form button { min-width:82px; }
