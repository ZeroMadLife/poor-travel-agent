...
const emit = defineEmits<{
  ...
  saveGoal: [description: string, completionCriteria: string[]]
  evaluateGoal: []
  continueGoal: []
  clearGoal: []
}>()
...
const goalCriteria = ref('')
const goalEditing = ref(false)
const goalSetupOpen = ref(false)
watch(
  () => [props.threadGoal, props.sessionTitle] as const,
  ([goal, title]) => {
    goalDescription.value = goal?.description || title
    goalCriteria.value = goal?.completion_criteria.join('\n') || ''
    goalEditing.value = false
    goalSetupOpen.value = false
  },
  { immediate: true },
)
...
function submitGoal() {
  ...
  const criteria = goalCriteria.value
    .split('\n')
    .map((item) => item.trim())
    .filter(Boolean)
  if (!description || !criteria.length) return
  emit('saveGoal', description, criteria)
  goalSetupOpen.value = false
  if (props.threadGoal) goalEditing.value = false
}
...
type LearningJourney = 'active' | 'review' | 'recovery'
...
const goalTitle = computed(() => props.threadGoal?.description || props.sessionTitle)
const goalActionDisabled = computed(() => (
  props.goalBusy || props.projection.status === 'running' || props.projection.status === 'blocked'
))
const continueDisabled = computed(() => goalActionDisabled.value || props.connectionState !== 'connected')
const blockerLabel = computed(() => {
  const blocker = props.threadGoal?.evaluation?.blocker
  return ({
  ...
  } as const)[blocker || 'goal_not_met_yet']
})
...
<template>
  ...
  >
    ...
    <div class="journey-canvas">
      ...
      <section v-if="journey === 'review'" class="journey-surface review-surface">
      ...
      <section v-else class="journey-surface active-surface">
        <header class="goal-heading">
          <div><p><Target :size="15" />{{ threadGoal ? 'Current Goal' : '主对话目标' }} <code v-if="threadGoal">rev {{ threadGoal.revision }}</code></p><h1>{{ goalTitle }}</h1><span>{{ threadGoal ? '当前 Thread 的学习路径；只有 timeline 与已验证证据会改变状态。' : '先完成对话，再决定是否绑定长期目标；运行事实会持续保留在本轮时间线。' }}</span></div>
          <div class="evidence-summary"><small>目标评估</small><strong>{{ threadGoal?.evaluation ? blockerLabel : '尚未评估' }}</strong><span>{{ threadGoal?.evaluation?.next_action || '等待本轮证据' }}</span></div>
        </header>

        <section v-if="!threadGoal" class="goal-empty" aria-label="长期目标状态">
          <div><ShieldCheck :size="17" /><span><strong>当前会话尚未绑定长期目标</strong><small>你可以先正常对话；绑定后，跨会话目标才会接收本轮已验证证据。</small></span></div>
          <button type="button" :disabled="goalActionDisabled" @click="openGoalSetup">绑定长期目标</button>
        </section>
        <form v-if="!threadGoal && goalSetupOpen" class="goal-contract-form goal-setup-form" @submit.prevent="submitGoal">
          <label><small>目标结果</small><textarea v-model="goalDescription" aria-label="目标结果" rows="2" maxlength="2000" /></label>
          <label><small>完成标准</small><textarea v-model="goalCriteria" aria-label="完成标准" rows="4" placeholder="每行一条可观察、可验证的标准" /></label>
          <button type="submit" :disabled="goalBusy || !goalDescription.trim() || !goalCriteria.trim()">{{ goalBusy ? '保存中' : '确认目标' }}</button>
        </form>
        <p v-if="goalError" class="goal-error" role="alert">{{ goalError }}</p>

        <div v-if="threadGoal" class="goal-actions" aria-label="目标操作">
          <button type="button" :disabled="goalActionDisabled" @click="goalEditing = !goalEditing">编辑目标</button>
          <button type="button" :disabled="goalActionDisabled" @click="emit('evaluateGoal')">评估本轮</button>
          <button type="button" class="primary" :disabled="continueDisabled" @click="emit('continueGoal')">继续目标</button>
          <button type="button" :disabled="goalActionDisabled" @click="emit('clearGoal')">清除目标</button>
          <span>{{ threadGoal.completion_criteria.length }} 条完成标准</span>
        </div>
        <form v-if="threadGoal && goalEditing" class="goal-contract-form goal-edit-form" @submit.prevent="submitGoal">
          <label><small>目标结果</small><textarea v-model="goalDescription" aria-label="编辑目标结果" rows="2" maxlength="2000" /></label>
          <label><small>完成标准</small><textarea v-model="goalCriteria" aria-label="编辑完成标准" rows="4" /></label>
          <button type="submit" :disabled="goalBusy || !goalDescription.trim() || !goalCriteria.trim()">保存修改</button>
        </form>
        <p v-if="goalError" class="goal-error" role="alert">{{ goalError }}</p>
...
.workbench-title { display:flex; align-items:center; gap:9px; min-width:0; }.workbench-mark { display:grid; place-items:center; flex:none; width:30px; height:30px; border:1px solid var(--sage-border); border-radius:var(--sage-radius); color:var(--sage-text-muted); background:var(--sage-surface-raised); }.workbench-mark.running,.workbench-mark.completed { color:var(--sage-success); }.workbench-mark.blocked { color:var(--sage-warning); }.workbench-mark.failed,.workbench-mark.cancelled { color:var(--sage-danger); }.workbench-title-copy { display:grid; min-width:0; }.workbench-title-copy strong { font-size:15px; }.workbench-title-copy span { max-width:240px; overflow:hidden; color:var(--sage-text-muted); font-size:12px; text-overflow:ellipsis; white-space:nowrap; }
.workbench-metrics { display:flex; align-self:stretch; min-width:0; margin:0 0 0 auto; }.workbench-metrics>div { display:grid; align-content:center; min-width:72px; padding:0 10px; border-left:1px solid var(--sage-border); }.workbench-metrics dt,.workbench-metrics dd { margin:0; }.workbench-metrics dt { display:flex; align-items:center; gap:4px; color:var(--sage-text-muted); font-size:11px; }.workbench-metrics dd { margin-top:2px; color:var(--sage-text-secondary); font-family:var(--sage-font-mono); font-size:13px; }.metric-state.running dd,.metric-state.completed dd { color:var(--sage-success); }.metric-state.blocked dd { color:var(--sage-warning); }.metric-state.failed dd,.metric-state.cancelled dd { color:var(--sage-danger); }.workbench-metrics .metric-context { min-width:140px; max-width:210px; }.metric-context dd { overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
...
.surface-heading p,.goal-heading p { display:flex; align-items:center; gap:7px; margin:0 0 7px; color:var(--sage-brand-strong); font-size:12px; font-weight:750; text-transform:uppercase; }.surface-heading h1,.goal-heading h1 { margin:0; color:var(--sage-text); font-size:clamp(21px,2.35cqw,30px); line-height:1.22; letter-spacing:0; }.surface-heading>span,.goal-heading>div>span { display:block; max-width:760px; margin-top:7px; color:var(--sage-text-muted); font-size:13px; line-height:1.55; }
.goal-heading p code { color:var(--sage-text-muted); font-family:var(--sage-font-mono); font-size:11px; font-weight:500; text-transform:none; }
.goal-heading { display:grid; grid-template-columns:minmax(0,1fr) 170px; align-items:start; gap:24px; padding-bottom:20px; border-bottom:1px solid var(--sage-border); }.evidence-summary { display:grid; align-content:center; min-height:72px; padding-left:16px; border-left:1px solid var(--sage-border); }.evidence-summary small,.evidence-summary span { color:var(--sage-text-muted); font-size:11px; }.evidence-summary strong { margin:3px 0; color:var(--sage-text-secondary); font-size:17px; }.evidence-summary span { margin:0; }
.goal-empty { display:flex; align-items:center; justify-content:space-between; gap:18px; min-height:68px; padding:12px 0; border-bottom:1px solid var(--sage-border); }.goal-empty>div { display:flex; align-items:center; gap:10px; min-width:0; }.goal-empty>div>svg { flex:none; color:var(--sage-brand-strong); }.goal-empty span { display:grid; min-width:0; }.goal-empty strong { font-size:13px; }.goal-empty small { margin-top:2px; color:var(--sage-text-muted); font-size:12px; }.goal-empty button { flex:none; min-height:34px; padding:0 12px; border:1px solid var(--sage-brand-strong); border-radius:var(--sage-radius); color:var(--sage-brand-strong); background:var(--sage-surface); font-size:12px; }.goal-empty button:hover { background:var(--sage-brand-bg); }.goal-empty button:disabled { cursor:not-allowed; opacity:.48; }
.goal-actions { display:flex; align-items:center; gap:8px; min-height:48px; border-bottom:1px solid var(--sage-border); }.goal-actions button,.goal-contract-form button { min-height:32px; padding:0 11px; border:1px solid var(--sage-border); border-radius:var(--sage-radius); color:var(--sage-text-secondary); background:var(--sage-surface); font-size:12px; cursor:pointer; }.goal-actions button.primary,.goal-contract-form button { border-color:var(--sage-brand-strong); color:var(--sage-surface); background:var(--sage-brand-strong); }.goal-actions button:disabled,.goal-contract-form button:disabled { cursor:not-allowed; opacity:.48; }.goal-actions>span { margin-left:auto; color:var(--sage-text-muted); font-size:11px; }.goal-error { margin:9px 0 0; color:var(--sage-danger); font-size:12px; }
...
.learning-path { display:grid; grid-template-columns:repeat(6,minmax(72px,1fr)); margin:0; padding:0; list-style:none; }.learning-path li { position:relative; display:grid; justify-items:center; align-content:start; gap:6px; min-width:0; text-align:center; }.path-node { position:relative; z-index:2; display:grid; place-items:center; width:38px; height:38px; border:2px solid var(--sage-border-strong); border-radius:50%; color:var(--sage-text-muted); background:var(--sage-surface); }.learning-path li.completed .path-node { border-color:var(--sage-success); color:var(--sage-surface); background:var(--sage-success); }.learning-path li.running .path-node,.learning-path li[aria-current="step"] .path-node { border-color:var(--sage-success); color:var(--sage-success); box-shadow:0 0 0 4px color-mix(in srgb,var(--sage-success) 12%,transparent); }.learning-path li.blocked .path-node { border-color:var(--sage-warning); color:var(--sage-warning); }.learning-path li.failed .path-node,.learning-path li.cancelled .path-node { border-color:var(--sage-danger); color:var(--sage-danger); }.learning-path strong { width:100%; overflow:hidden; font-size:13px; text-overflow:ellipsis; white-space:nowrap; }.learning-path small { width:100%; overflow:hidden; color:var(--sage-text-muted); font-size:11px; text-overflow:ellipsis; white-space:nowrap; }.learning-path li>i { position:absolute; z-index:1; top:18px; left:calc(50% + 19px); width:calc(100% - 38px); height:1px; background:var(--sage-border-strong); }.learning-path li.completed>i { background:var(--sage-success); }.learning-path li.running .path-node>svg { animation:path-spin 1.2s linear infinite; }
.active-grid,.recovery-grid { display:grid; grid-template-columns:minmax(0,1.15fr) minmax(240px,.85fr); gap:28px; padding:20px 0 16px; }.active-grid h2,.recovery-grid h2,.runtime-resources h2 { margin:0 0 10px; font-size:14px; }.event-replay ul,.preserved-context ul { margin:0; padding:0; list-style:none; }.event-replay li { display:grid; grid-template-columns:74px 15px minmax(0,1fr) auto; align-items:center; gap:8px; min-height:40px; border-top:1px solid var(--sage-border); color:var(--sage-text-muted); font-size:13px; }.event-replay time,.event-replay code { font-family:var(--sage-font-mono); font-size:11px; }.event-replay li>svg { color:var(--sage-success); }.event-replay span { overflow:hidden; color:var(--sage-text-secondary); text-overflow:ellipsis; white-space:nowrap; }
...
.goal-contract-form { display:grid; grid-template-columns:minmax(0,1fr) minmax(0,1fr) auto; align-items:end; gap:12px; padding:18px 0; border-bottom:1px solid var(--sage-border); }.goal-contract-form label { display:grid; gap:6px; }.goal-contract-form small { color:var(--sage-text-muted); font-size:11px; }.goal-contract-form textarea { width:100%; min-height:72px; resize:vertical; padding:10px 11px; border:1px solid var(--sage-border); border-radius:var(--sage-radius); color:var(--sage-text); background:var(--sage-surface); font:inherit; font-size:13px; line-height:1.55; box-sizing:border-box; }.goal-contract-form textarea:focus { border-color:var(--sage-brand-strong); outline:none; }.goal-contract-form button { min-width:88px; }
...
@container coding-harness (max-width:900px) { .metric-context { display:none !important; }.active-grid,.recovery-grid { grid-template-columns:1fr; }.workbench-title-copy span { max-width:140px; }.contract-fields,.goal-contract-form { grid-template-columns:1fr; }.contract-fields>div { border-right:0; border-bottom:1px solid var(--sage-border); }.contract-fields>div:last-child { border-bottom:0; }.goal-contract-form button { justify-self:start; } }
@media (max-width:767px) { .coding-harness-workbench { grid-template-rows:auto minmax(0,1fr); }.workbench-header { display:none; }.journey-surface { padding:66px 14px 74px; }.surface-heading h1,.goal-heading h1 { font-size:22px; }.goal-heading { grid-template-columns:1fr; gap:14px; }.evidence-summary { min-height:0; padding:10px 0 0; border-top:1px solid var(--sage-border); border-left:0; }.goal-empty { align-items:flex-start; flex-direction:column; gap:10px; }.goal-empty button { width:100%; }.goal-actions { flex-wrap:wrap; padding:8px 0; }.goal-actions>span { width:100%; margin-left:0; }.path-section>header span { display:none; }.learning-path { grid-template-columns:repeat(3,minmax(0,1fr)); gap:22px 4px; }.learning-path li>i { display:none; }.active-grid,.recovery-grid { gap:20px; }.runtime-resources { display:none; }.canvas-status { display:none; }.contract-fields,.goal-contract-form { grid-template-columns:1fr; }.contract-fields>div { min-height:86px; border-right:0; border-bottom:1px solid var(--sage-border); }.artifact-strip { grid-template-columns:1fr; }.artifact-strip>span { border-right:0; border-bottom:1px solid var(--sage-border); }.artifact-strip>span:last-child { border-bottom:0; }.surface-guidance { align-items:flex-start; } }
