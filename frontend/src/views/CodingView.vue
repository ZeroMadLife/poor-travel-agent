...
import { ArrowDown, Eye, EyeOff, FileText, House, Menu, ScrollText, Settings, X } from 'lucide-vue-next'
import {
  CodingApprovalCard,
  CodingComposer,
  CodingDiffDrawer,
    ...
    <div class="chat-shell" :class="{ compact: isCompact }">
      ...
      <main class="pane-center" :class="{ 'is-inert': mainInert }" :inert="mainInert || undefined" :aria-hidden="mainInert ? 'true' : undefined">
        <header class="session-titlebar">
          ...
          <div class="titlebar-actions"><RouterLink class="home-link" to="/assistant" aria-label="返回今天" title="返回今天"><House :size="16" /></RouterLink><button class="files-toggle" type="button" aria-label="打开文件" title="打开文件" @click="openFilesDrawer"><FileText :size="16" /></button><CodingGitBadge /></div>
        </header>
