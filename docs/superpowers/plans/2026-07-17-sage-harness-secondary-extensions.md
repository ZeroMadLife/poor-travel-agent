...
回执，前端从 timeline 投影“检索门限”资源。该切片当时仅提供可观测建议；H2.8B 已在此
基础上完成本轮来源可见性和恢复路由，安全边界仍由各 Port、approval 与 sandbox 强制执行。
...
实现状态（2026-07-22）：已完成 H2.8B 最小闭环。`MemoryPort.query_context` 按来源接受
独立 token budget；Semantic 仅检索已批准且与 query 相关的事实，潜在冲突以原子组返回，
预算不足时不会只暴露冲突一侧；Episodic 从 durable timeline 生成受限运行摘要和 evidence
references，原始用户 prompt 仅参与服务端匹配，不进入模型可见摘要。每次召回持久化
memory/episode ID、revision、时间、provenance、命中/省略/不可用来源及预算消耗 receipt。

Gate 选择会落实到当轮 Knowledge、Web、Research 子代理、MCP 网络能力和本地 Shell 网络
回退的可见性；恢复时从 durable `retrieval_gate_decided` receipt 重建同一路由。生产容器仍
以 `--network none` 为最终网络边界，本地策略检查用于保持开发环境一致。显式空来源或损坏
的 selected sources 均 fail closed；为兼容 H2.8A 之前的历史 run，完全缺失 Gate receipt 的
旧 checkpoint 仍按 legacy 路由恢复。H2.8B 不自动沉淀 episode 或事实，长期写入继续进入
H2.8C proposal/approval。
