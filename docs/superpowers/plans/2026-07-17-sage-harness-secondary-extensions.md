...
> 日期：2026-07-17
>
> 状态：执行中；H2.5B2 已完成本地门禁，待 PR 合入 `dev/sage-v7`
>
...
第一批次没有加入 Web Search、Memory 或自动 Goal 续跑，先建立了后续扩展共享的能力底座。随后 H2.4B-C 已完成统一发现、安全提升、健康度与审计；H2.5A 已交付 Search 与当前回答引用，H2.5B0-B2 已依次交付运行预算与 Shell 可靠性、安全 Fetch/Artifact、异步 PDF 解析。下一独立切片是 H2.5C 的用户确认来源提案；H2.6 Research Subagent 仍在其后。
