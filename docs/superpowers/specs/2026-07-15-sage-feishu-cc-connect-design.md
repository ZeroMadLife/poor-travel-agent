...
- 使用 `cc-connect web` 的本地管理页面录入应用凭据，避免把 App Secret 放入
  命令行参数和 shell 历史。
...
实际的 `app_id` 和 `app_secret` 由用户在本地 Web UI 中填写，不出现在本文档中。
...
2. 启动 `cc-connect web`，创建名为 `sage` 的项目并录入飞书凭据。
3. 启动 `cc-connect` 前台进程，确认飞书平台和 WebSocket 长连接启动成功。
4. 将机器人加入目标飞书群。
5. 在群中发送只读测试请求：
...
6. 验证回复内容来自 Sage 仓库，并确认没有工作区文件变化。
