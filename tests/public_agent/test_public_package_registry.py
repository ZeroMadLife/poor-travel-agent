...
def test_bootstrap_stages_and_activates_an_immutable_package(tmp_path: Path) -> None:
    ...
    assert result["active_revision"] == "2026-07-24.1"
    assert PublishedPackageProvider(tmp_path).current().revision == "2026-07-24.1"
    ...
    assert (tmp_path / "packages/sage-public/2026-07-24.1.json").stat().st_mode & 0o777 == 0o644
...
def test_publish_switches_active_revision_and_provider_reloads_without_restart(
    ...
    assert provider.current().revision == "2026-07-24.1"
    registry.stage_payload(_payload("2026-07-24.2", suffix=" P3"), actor="sage-deploy")
    ...
    assert states == {"2026-07-24.1": "inactive", "2026-07-24.2": "active"}
...
def test_revoke_active_revision_atomically_restores_previous_healthy_revision(
    ...
    assert result["replacement_revision"] == "2026-07-24.1"
    assert result["active_revision"] == "2026-07-24.1"
    assert PublishedPackageProvider(tmp_path).current().revision == "2026-07-24.1"
    state = json.loads((tmp_path / "registry.json").read_text(encoding="utf-8"))
    assert state["events"][-1]["reason"] == "资料需要重新审核"
    assert state["events"][-1]["replacement_revision"] == "2026-07-24.1"
...
def test_revoke_only_rolls_back_to_the_same_package_id(tmp_path: Path) -> None:
    ...
    registry.stage_payload(_payload("2026-07-24.2", suffix=" P3"), actor="root")
    registry.activate(
        "sage-public",
        "2026-07-24.2",
...
def test_same_revision_with_different_content_cannot_overwrite_immutable_file(
    ...
    with pytest.raises(PublishedPackageError, match="禁止覆盖"):
        registry.stage_payload(_payload("2026-07-24.1", suffix=" changed"), actor="root")
