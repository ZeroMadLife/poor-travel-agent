...
def create_anthropic_llm(model_spec: str, temperature: float = 0.0, **kwargs: Any) -> ChatAnthropic:
    ...
    if provider == "deepseek_anthropic":
        ...
        if not api_key:
            raise ValueError("LLM provider 'deepseek_anthropic' 的 API key 未配置（环境变量: DEEPSEEK_API_KEY）")
        ...
        if not api_key:
            raise ValueError("LLM provider 'anthropic' 的 API key 未配置（环境变量: ANTHROPIC_API_KEY）")
        ...
        raise ValueError(f"非 Anthropic provider: {provider}（支持: anthropic, deepseek_anthropic）")
